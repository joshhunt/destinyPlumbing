 ...
 * @return {Object}
 */
objectDiff.diff = function diff(a, b, cb) {
  if (a === b) {
    return {
      changed: 'equal',
      value: a,
  ...
  for (var key in a) {
    if (key in b) {
      if (a[key] === b[key]) {
        ...
        if (
          ...
          var valueDiff = diff(a[key], b[key], cb);
    ...
    if (cb) {
      const newResult = cb(value[key], key, a[key], b[key]);
      if (newResult) {
        value[key] = newResult;
      }

      if (newResult && newResult.changed === 'equal') {
        equal = true;
      }
    }
  }
    ...
    for (var key in diff) {
      ...
      switch (changed) {
        case 'removed':
          properties.push(
            '<del class="diff">' +
              stringifyObjectKey(escapeHTML(key)) +
              '<span>: </span>' +
              inspect(diff[key].value) +
              '</del>',
          );
          break;
        ...
        case 'primitive change':
          ...
          properties.push(
            '<del class="diff diff-key">' +
              prefix +
              inspect(diff[key].removed) +
              '</del><span>,</span>\n' +
              '<ins class="diff diff-key">' +
              prefix +
              inspect(diff[key].added) +
              '</ins>',
          );
          break;
